...
local function ScanSingleAchievement(id, isCompleted, month, day, year)
	...
	if isCompleted then
		local completed = addon.ThisCharacter.Completed
		local bitPos = (id % 32)	
	...
	if num == 1 then
		...
		if quantity and quantity > 0 then
			addon.ThisCharacter.Partial[id] = quantity
		end
		return 
	end
	...
	if #CriteriaCache > 0 then		-- if at least one criteria completed, save the entry, do nothing otherwise
		addon.ThisCharacter.Partial[id] = table.concat(CriteriaCache, ",")
	end
end
...
local function _GetAchievementInfo(character, achievementID)
	...
	if character.Partial[achievementID] then
...
local function _GetCriteriaInfo(character, achievementID, criteriaIndex)
	local achievement = character.Partial[achievementID]
