...
local bsize = (((140) - (4 * 7)) / 6)

Anchorraidbuff_reminder = CreateFrame("Frame","Move_raidbuff_reminder",UIParent)
Anchorraidbuff_reminder:SetPoint("LEFT", bottompanel, "LEFT", 5, 50)
CreateAnchor(Anchorraidbuff_reminder, "Move Raid Buff Reminder", bsize + 8, 142)

--Create the Main bar
local raidbuff_reminder = CreateFrame("Frame", "RaidBuffReminder", UIParent)
CreatePanel(raidbuff_reminder, bsize + 8,142 , "BOTTOMRIGHT", Anchorraidbuff_reminder)
...
CreateShadow(raidbuff_reminder)

--Function to create buttons
local function CreateButton(name, relativeTo, firstbutton)
	local button = CreateFrame("Frame", name, RaidBuffReminder)
	if firstbutton == true then
		CreatePanel(button, bsize, bsize, "TOP", relativeTo, "BOTTOM", 0, 23)
	else
		CreatePanel(button, bsize, bsize, "BOTTOM", relativeTo, "TOP", 0, 4)
	end
