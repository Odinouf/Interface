...
local spacing = 1
local maxbars = 8
local width, height = 220, maxbars*(barheight+spacing)-spacing
local maxfights = 10
local reportstrings = 10
local texture = "Interface\\Addons\\QulightUI\\media\\Other\\statusbar3"
	...
	if timer > 0.5 then
		for i, v in pairs(current) do
			...
			if i == UnitGUID("player") then
				...
				if pr == "TANK" then 
					dataobj.text = string.format("Avoid: %.2f%%", GetBlockChance() + GetDodgeChance() + GetParryChance() + 5)
				elseif pr == "HEALER" then 
					dataobj.text = string.format("HPS: %.0f", v[SHOW_COMBAT_HEALING].amount / v.combatTime)
				else
...
local OnEvent = function(self, event, ...)
	...
	elseif event == "VARIABLES_LOADED" then
		if not hidetitle then
			...
			MainFrame.title:SetPoint("BOTTOM", MainFrame, "TOP", 0, 0)
			MainFrame.title:SetText(sMode)
...
function dataobj.OnEnter(self)
	...
	GameTooltip:AddLine("Click to switch between Damage/Healing mode.")
	GameTooltip:Show()
end
