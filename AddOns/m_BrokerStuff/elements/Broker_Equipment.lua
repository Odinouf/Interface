...
f:SetScript("OnEvent",function() PickupInventoryItem(17) PutItemInBackpack() UseEquipmentSet(Broker_EquipmentDB.spec["spec"..GetActiveTalentGroup()]) end)
...
StaticPopupDialogs["CONFIRM_APPOINT_SET"] = {
  ...
  OnAccept = function(self)
	local set=self.data
    local spc="spec"..GetActiveTalentGroup()
	local _, spcname = GetTalentTabInfo(GetPrimaryTalentTree(false, false, GetActiveTalentGroup()), false, false)
	if set~="" then
