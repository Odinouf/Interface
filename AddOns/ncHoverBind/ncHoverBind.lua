	...
	if not bind.loaded then
		...
		function bind:Update(b, spellmacro)
			...
			if spellmacro=="SPELL" then
				self.button.id = SpellBookFrame_GetSpellBookSlot(self.button) 
				self.button.name = GetSpellBookItemName(self.button.id, SpellBookFrame.bookType) -- GetSpellName replaced with GetSpellBookItemName
		...
		function bind:Listener(key)
			...
			if #self.button.bindings > 0 then
				...
				self:Update(self.button, self.spellmacro)
				if self.spellmacro~="MACRO" then GameTooltipHide() end
			end
