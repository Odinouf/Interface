...
if not libTab or libTab.Version < version then
	if ( libTab ) then local localData = libTab.Data else localData = {} end
	...
	libTab = {
		...
		tabOrderedPairs = function (self, t, o)
			local a = {}
				...
				for k, v in pairs(t) do a[v[o]] = k end
				local i = 0
				local iter = function ()
					i = i + 1
		...
		tabFilteredPairs = function (self, t, f)
			local a = {}
				for k, v in pairs(t) do if ( type(k) == f ) then table.insert(a, k) end end
				local i = 0
				local iter = function ()
					i = i + 1
					if ( a[i] == nil )  then return nil
		...
		tabHighlight = function (tabsId, tab)
			for key, value in libTab:tabFilteredPairs(libTab.Data[tabsId].tab, "number") do
				if ( tab == key ) then
					_G["libTab"..tabsId..key]:SetChecked(1)
				else
					_G["libTab"..tabsId..key]:SetChecked(0)
				end
			end
		end,
		...
		tabOnShow = function (self, tabsId, ...)
			...
			local gId = libTab.Data[tabsId].tab["gid_"..caller.Frame]
			local group
				if ( gId ) then group = libTab.Data[tabsId].tab["group_"..gId] else group = nil end

			if ( gId and group) then
				if ( group ~= caller.Frame and _G[group]:IsShown() ) then
					if ( caller.OnShow ) then
						caller.OnShow(self, ...)
					end
					for key, value in libTab:tabFilteredPairs(libTab.Data[tabsId].tab, "number") do
						HideUIPanel(_G[value.Frame])
					end
					return
				end
			end
			...
			if ( not _G[caller.Frame]:IsShown() ) then
				ShowUIPanel(_G[caller.Frame])
			end

			libTab.Data[tabsId].parent:SetParent(caller.Frame)
			libTab.Data[tabsId].parent:ClearAllPoints()
			libTab.Data[tabsId].parent:SetPoint("TOPLEFT", caller.Frame, "TOPRIGHT", caller.offsetX, libTab.TabOffsetY+caller.offsetY)
			libTab.tabHighlight(tabsId, tab)
		...
		tabHook = function (self, tabsId)
			for key in libTab:tabFilteredPairs(libTab.Data[tabsId].tab, "number") do
				libTab.Data[tabsId].tab[key]["OnShow"] = _G[libTab.Data[tabsId].tab[key].Frame]:GetScript("OnShow") or nil
				_G[libTab.Data[tabsId].tab[key].Frame]:SetScript("OnShow", function(self, ...) libTab.tabOnShow(self, tabsId, ...) end)
				libTab.Data[tabsId].tab[key]["OnHide"] = _G[libTab.Data[tabsId].tab[key].Frame]:GetScript("OnHide") or nil
				_G[libTab.Data[tabsId].tab[key].Frame]:SetScript("OnHide", function(self, ...) libTab.tabOnHide(self, tabsId, ...) end)
			end
		end,
		...
		initialize = function (self, tabsId, tabsTable)
			...
			for key, value in libTab:tabOrderedPairs(tabsTable, "order") do
				...
				i = i + 1
				libTab.Data[tabsId].tab[i] = CreateFrame("CheckButton", "libTab"..tabsId..i, libTab.Data[tabsId].parent, "libTabtabTemplate")
				...
				if ( i == 1 ) then
					...
					libTab.Data[tabsId].tab[i]:ClearAllPoints()
					libTab.Data[tabsId].tab[i]:SetPoint("TOPLEFT", libTab.Data[tabsId].tab[lastTab], "BOTTOMLEFT", 0, -17)
				end
				--libTab.Data[tabsId].tab[i]:SetNormalTexture(value.Texture) -- no longer used, creating our own texture object now
				libTab.Data[tabsId].tab[i].Texture:SetTexture(value.Texture)
				...
				libTab.Data[tabsId].tab[i].offsetY = value.offsetY or 0
					...
					UIPanelWindows[value.Frame]["width"] = _G[value.Frame]:GetWidth()+(libTab.TabOffsetX+value.offsetX)
				...
				libTab.Data[tabsId].tab["gid_"..value.Frame] = value.group or nil
					if ( value.group ) then libTab.Data[tabsId].tab["group_"..value.group] = nil end
